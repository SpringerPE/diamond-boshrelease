...
for package_bin_dir in $(ls -d /var/vcap/jobs/$JOB_NAME/packages/*/*bin)
do
...
for python_mod_dir in $(ls -d /var/vcap/jobs/$JOB_NAME/packages/*/lib/python*/site-packages 2>/dev/null)
do
  export PYTHONPATH=${python_mod_dir}:$PYTHONPATH
done

export LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-''} # default to empty
for package_bin_dir in $(ls -d /var/vcap/jobs/$JOB_NAME/packages/*/lib)
do
  export LD_LIBRARY_PATH=${package_bin_dir}:$LD_LIBRARY_PATH
done
for package_bin_dir in $(ls -d /var/vcap/jobs/$JOB_NAME/packages/*/lib/python*/lib-dynload 2>/dev/null)
do
